...
from .serializers import (
    ...
    PayrollEntrySerializer, FinancialReportSerializer
)
...
class PayrollPeriodListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        # Define a data de referência como o último dia do mês
...
def close_payroll_period(request, period_id):
    ...
    try:
        ...
        if period.is_closed:
            return Response(
                {'error': 'Este período já está fechado'}, 
...
def generate_payroll_entries(request, period_id):
    """Gera entradas de folha para todos os funcionários ativos"""
    try:
        period = PayrollPeriod.objects.get(id=period_id)
        ...
        for employee in active_employees:
            # Verifica se já existe entrada para este funcionário no período
...
def generate_monthly_closing(request):
    ...
    try:
        ...
        # Estimativa de despesas operacionais (30% da receita)
        ...
        report = FinancialReport.objects.create(
            ...
            report_data={
                ...
                'revenue_breakdown': {
                    'completed_jobs': float(total_revenue),
                    'jobs_count': completed_jobs_count
                },
                'expense_breakdown': {
        ...
        return Response({
            'message': 'Fechamento mensal gerado com sucesso',
            'report': FinancialReportSerializer(report).data
        })
        
    except Exception as e:
        return Response(
            {'error': f'Erro ao gerar fechamento: {str(e)}'}, 
...
def generate_annual_balance(request):
    ...
    try:
        ...
        payroll_expenses = payroll_periods.aggregate(
            Sum('total_net_salary')
        )['total_net_salary__sum'] or Decimal('0.00')
        ...
        for month in range(1, 13):
            ...
            month_jobs = jobs_year.filter(
                created_at__date__gte=month_start,
                created_at__date__lte=month_end
            )
            
            month_revenue = month_jobs.filter(status='completed').aggregate(
                Sum('total_price')
            )['total_price__sum'] or Decimal('0.00')
            
            try:
                month_payroll = PayrollPeriod.objects.get(year=year, month=month)
                month_payroll_expense = month_payroll.total_net_salary
            except PayrollPeriod.DoesNotExist:
            ...
            monthly_data.append({
                ...
                'payroll_expense': float(month_payroll_expense),
                'jobs_count': month_jobs.count(),
                'completed_jobs': month_jobs.filter(status='completed').count()
            })
        ...
        return Response({
            'message': 'Balanço anual gerado com sucesso',
            'report': FinancialReportSerializer(report).data
        })
        
    except Exception as e:
        return Response(
            {'error': f'Erro ao gerar balanço: {str(e)}'}, 
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
